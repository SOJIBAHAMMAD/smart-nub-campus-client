...
interface AddMembersModalProps {
  ...
  /** Called after members are added so the UI can refresh. */
  onAdded?: () => void;
}

/**
 * Modal to add new members to an existing group conversation. Existing members
...
export function AddMembersModal({
  ...
  }, [query, open, currentUserId, conversation]);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success(`Added ${selected.length} member${selected.length === 1 ? "" : "s"}`);
      onOpenChange(false);
      onAdded?.();
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Add people to &quot;{conversation.name ?? "this group"}&quot;.
          </DialogDescription>
        </DialogHeader>
